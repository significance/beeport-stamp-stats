...
- **Database name:** `beeport3m_testing`
- **Source database:** `beeport3m` (production/main database)
- **Reset procedure:** Always recreate `beeport3m_testing` from `beeport3m` at the start of every test run

**CRITICAL: User Confirmation Required**

Before creating or resetting the testing database, Claude MUST:

1. **Always ask before copying from beeport3m:**
   - Check if `beeport3m` database exists
   - Ask user for explicit permission before copying data to `beeport3m_testing`
   - Example: "I need to reset the testing database. May I copy beeport3m to beeport3m_testing?"

2. **If beeport3m does not exist:**
   - Notify the user that the source database doesn't exist
   - Ask if they want Claude to create a fresh empty database
   - Example: "The beeport3m database doesn't exist. Would you like me to create a new empty beeport3m_testing database for testing?"

**Standard test database setup:**
```bash
# Check if source database exists first
psql -lqt | cut -d \| -f 1 | grep -qw beeport3m

# If it exists, drop and recreate testing database from production data
psql -c "DROP DATABASE IF EXISTS beeport3m_testing;"
psql -c "CREATE DATABASE beeport3m_testing TEMPLATE beeport3m;"

# If beeport3m doesn't exist, create fresh empty testing database
psql -c "DROP DATABASE IF EXISTS beeport3m_testing;"
psql -c "CREATE DATABASE beeport3m_testing;"
```
...
./target/release/beeport-stamp-stats \
  --database-url "postgresql://localhost/beeport3m_testing" \
...
DB="--database-url postgresql://localhost/beeport3m_testing"
...
database:
  url: "postgresql://localhost/beeport3m_testing"
blockchain:
  chunk_size: 1000
EOF
...
BEEPORT__DATABASE__URL="postgresql://localhost/beeport3m_env_test" \
  ./target/release/beeport-stamp-stats --config test-config.yaml fetch --from-block <start> --to-block <end>
