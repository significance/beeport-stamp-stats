...
def fetch_and_plot_metrics():
    ...
    try:
        ...
        # Figure setup with dark facecolors
        fig, ax_main = plt.subplots(figsize=(16, 9), dpi=100, facecolor='#121212')
        ...
        for i, (label, df) in enumerate(dataframes.items()):
            ...
            if label == "Price":
                ...
                item = ax.scatter(plot_data.index, plot_data.iloc[:, 0], color=color, label=label, s=45, edgecolors='none', alpha=0.8)
                lines_and_scatters.append(item)
            
            # Styling axes for dark mode
            ax.set_ylabel(label, color=color, fontweight='bold', fontsize=9)
            ax.tick_params(axis='y', labelcolor=color, colors='#888888')
        ...
        plt.title(f'Storage Incentives Metrics (Interactive Zoom Enabled)\nDatabase: {dbname}', 
                  fontsize=14, pad=25, color='white')
        
        labels = [obj.get_label() for obj in lines_and_scatters]
        # Dark legend styling
        legend = ax_main.legend(lines_and_scatters, labels, loc='upper left', frameon=True, shadow=False)
        ...
        ax_main.grid(True, which='both', linestyle='--', linewidth=0.5, color='#333333', alpha=0.5)
        
        print("Graph generated in Dark Mode. Use the magnifying glass icon in the window to zoom.")
        plt.show(block=True) 
